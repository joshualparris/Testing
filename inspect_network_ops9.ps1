...
$Global:MockGetNetIPAddressCalls = New-Object System.Collections.Generic.List[object]
...
Set-MockFunction -Name Get-NetIPAddress -Body {
    ...
    $Global:MockGetNetIPAddressCalls.Add([pscustomobject]@{ InterfaceAlias=$InterfaceAlias; IPAddress=$IPAddress; AddressFamily=$AddressFamily; HasIPAddress = $PSBoundParameters.ContainsKey('IPAddress') }) | Out-Null
...
Write-Output "Get-NetIPAddress called $($Global:MockGetNetIPAddressCalls.Count) times"
$Global:MockGetNetIPAddressCalls | ForEach-Object { Write-Output ($_ | Out-String) }
